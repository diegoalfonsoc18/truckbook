...
function getEstado(fecha: string | null) {
  ...
  if (diff < 0) return { label: `Vencido hace ${Math.abs(diff)} días`, color: "#E94560", bg: "#E9456015" };
...
export default function SoatVehiculos() {
  ...
  const cargarDatos = useCallback(async () => {
    ...
    const { data: db } = await supabase
      .from("vehiculos")
      .select("placa, tipo_camion, soat_vencimiento")
      .in("placa", placas);

    const resultado = (db || []).map((v) => ({
      placa: v.placa,
      tipo_camion: v.tipo_camion || "estacas",
      soat_vencimiento: v.soat_vencimiento || null,
    }));

    resultado.sort((a, b) => {
      const ea = getEstado(a.soat_vencimiento);
      const eb = getEstado(b.soat_vencimiento);
  ...
  const abrirEditar = (v: VehiculoSoat) => {
    setEditPlaca(v.placa);
    setEditFecha(v.soat_vencimiento || "");
  ...
  const handleGuardar = async () => {
    ...
    const { error } = await supabase
      .from("vehiculos")
      .update({ soat_vencimiento: editFecha || null })
      .eq("placa", editPlaca);
  ...
  const vencidos = vehiculos.filter((v) => getEstado(v.soat_vencimiento).color === "#E94560").length;
  const porVencer = vehiculos.filter((v) => getEstado(v.soat_vencimiento).color === "#FFB800").length;
    ...
    <View style={[styles.container, ds.container]}>
      <SafeAreaView style={styles.safeArea} edges={["top"]}>
        <View style={styles.headerSection}>
          <View style={styles.header}>
            ...
            <View style={{ flex: 1, marginLeft: 12 }}>
              <Text style={[styles.headerTitle, ds.text]}>SOAT</Text>
          ...
          <View style={styles.resumenRow}>
            {vencidos > 0 && (
              <View style={[styles.chip, { backgroundColor: "#E9456015", borderColor: "#E9456040" }]}>
                <Text style={[styles.chipText, { color: "#E94560" }]}>{vencidos} vencido{vencidos > 1 ? "s" : ""}</Text>
              </View>
            )}
            {porVencer > 0 && (
        ...
        <FlatList
          ...
          renderItem={({ item }) => {
            const estado = getEstado(item.soat_vencimiento);
            return (
              <TouchableOpacity style={[styles.card, ds.cardBg, getShadow(isDark, "sm")]} onPress={() => abrirEditar(item)} activeOpacity={0.7}>
                ...
                <View style={[styles.estadoRow, { backgroundColor: estado.bg }]}>
                  <View>
                    ...
                    <Text style={[styles.fechaText, ds.textMuted]}>Vence: {formatFecha(item.soat_vencimiento)}</Text>
                  </View>
            ...
            <View style={[styles.modalOverlay, { backgroundColor: colors.overlay }]}>
              <TouchableWithoutFeedback>
                <View style={[styles.modalContent, ds.modalBg]}>
                  ...
                  <Text style={[styles.modalTitle, ds.text]}>SOAT</Text>
